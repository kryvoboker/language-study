...
<template>
    <div class="card">
        <div class="flex flex-wrap items-center gap-2 border-b border-base-content/10 px-4 py-3 sm:px-5">
            <select v-model="sourceLanguage" class="select select-sm w-40" @change="schedule">
            ...
            <select v-model="targetLanguage" class="select select-sm w-40" @change="schedule">
                <option v-for="lang in languages" :key="lang.code" :value="lang.code">{{ lang.label }}</option>
            </select>
        ...
        <div class="workspace-grid">
            <div class="text-panel border-b border-base-content/10 p-5 lg:border-e lg:border-b-0 sm:p-7">
                <div class="mb-4 flex items-center justify-between">
                    <div><p class="text-xs font-semibold uppercase tracking-[0.18em] text-base-content/45">{{ $t('translator.source') }}</p>
                        <h2 class="mt-1 text-lg font-medium">{{ $t('translator.write') }}</h2></div>
                    <span class="badge badge-soft" :class="sourceText.length > inputCharacterLimit ? 'badge-error' : 'badge-neutral'">{{ sourceText.length }}/{{ inputCharacterLimit }}</span></div>
                ...
                <div class="mt-4 flex items-center justify-between border-t border-base-content/10 pt-4"><p class="max-w-xl text-xs leading-5 text-base-content/50">{{ $t('translator.privacyHint') }}</p>
                    <button type="button" class="btn btn-primary btn-sm" :disabled="!sourceText.trim() || isBusy" @click="translate"><span class="icon-[tabler--sparkles] size-4"/>{{ $t('translator.translate') }}</button>
                </div>
            </div>

            <div class="text-panel p-5 sm:p-7">
                <div class="mb-4 flex items-center justify-between">
                    <div><p class="text-xs font-semibold uppercase tracking-[0.18em] text-primary">{{ $t('translator.translation') }}</p>
                        <h2 class="mt-1 text-lg font-medium">{{ $t('translator.result') }}</h2></div>
                    <button v-if="current?.translation" type="button" class="btn btn-circle btn-text btn-sm" @click="copyTranslation"><span class="icon-[tabler--copy] size-4"/></button>
                </div>
                <div v-if="isBusy" class="space-y-3 pt-3">
                    <div class="skeleton h-6 w-11/12"/>
                    <div class="skeleton h-6 w-9/12"/>
                    <div class="skeleton h-6 w-10/12"/>
                </div>
                <p v-else-if="current?.translation" class="whitespace-pre-wrap text-xl leading-relaxed">{{ current.translation }}</p>
                <div v-else class="grid h-64 place-items-center text-center text-base-content/35">
                    <div><span class="icon-[tabler--text-recognition] mx-auto mb-3 size-8"/>
                        <p>{{ $t('translator.empty') }}</p></div>
                </div>
                <div v-if="errorMessage" class="alert alert-soft alert-error mt-4">
                    <div>
                        <p>{{ errorMessage }}</p>
                        <NuxtLink v-if="accessError === 'loginRequired'" :to="localePath('/login')" class="link link-primary mt-2 inline-block">
                            {{ $t('nav.login') }}
                        </NuxtLink>
                    </div>
                </div>
